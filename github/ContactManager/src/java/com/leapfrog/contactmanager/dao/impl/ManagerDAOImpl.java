...
public class ManagerDAOImpl implements ManagerDAO {
    ...
    public int insert(Manager m) throws ClassNotFoundException, SQLException {
        String sql = "INSERT INTO tbl_contactmanager(name,email,phone,address,dob) values(?,?,?,?,?)";
        db.open();
        PreparedStatement stmt = db.initStatement(SQLConstant.INSERT_MANAGER);
        stmt.setString(1, m.getName());
        stmt.setString(2, m.getEmail());
        stmt.setString(3, m.getPhone());
        stmt.setString(4, m.getAddress());
        stmt.setDate(5, new java.sql.Date(m.getDOB().getTime()));
    ...
    public int update(Manager m) throws ClassNotFoundException, SQLException {
        ...
        stmt.setString(5, m.getDOB().toString());
        int result = db.executeUpdate();
        db.close();
        return result;
